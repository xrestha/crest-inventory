...
// Edits one employee's pay (basic / allowances / deductions / SSF) + bank details.
// Updates only those columns on hr_employees + syncs hr_salary_components (delete-all + re-insert).
export default function PayForm({ employee, onSave, onClose }) {
  ...
  async function handleSave() {
    ...
    const rows = components.filter(c => c.name.trim()).map(c => ({
      ...
      <div style={{
        ...
        <div style={{ flex: 1, padding: 24, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {tab === 'salary' && <>
            <div style={col}>
              <label style={lbl}>
                <Tip text="Monthly — fixed salary. Daily / Hourly — paid per day or hour worked; actual pay is computed from attendance in Payroll." width={300}>Pay Basis</Tip>
              </label>
            ...
            <div style={{ background: '#0f1117', border: '1px solid #2a2f3d', borderRadius: 8, overflow: 'hidden' }}>
              <button onClick={() => setSplitOpen(o => !o)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', padding: '10px 14px', cursor: 'pointer', color: '#9ca3af', fontSize: 12 }}>
                <span>
                  <Tip text="Enter a total/gross salary and we'll split it into basic and allowances. You can edit everything afterwards." width={300}>⚡ Split from gross salary</Tip>
                </span>
              ...
              {splitOpen && (
                <div style={{ padding: '0 14px 14px' }}>
                  ...
                  <button onClick={applySplit} disabled={!(parseFloat(grossInput) > 0)} className="btn btn-primary" style={{ marginTop: 10, fontSize: 12, opacity: parseFloat(grossInput) > 0 ? 1 : 0.5 }}>
                    Apply split
                  </button>
            ...
            {!isMonthly && (
              <div style={{ padding: '14px 16px', background: '#0f1117', borderRadius: 8, border: '1px solid #2a2f3d', fontSize: 12, color: '#6b7280', lineHeight: 1.6 }}>
                This employee is paid <strong style={{ color: '#e8e0d0' }}>per {payUnit}</strong>. Their actual pay each period is calculated from days/hours worked via the <strong style={{ color: '#9ca3af' }}>Attendance</strong> and <strong style={{ color: '#9ca3af' }}>Payroll</strong> modules. Monthly allowances, deductions, and SSF are not configured here for hourly/daily workers.
              </div>
            )}
            ...
            <div style={{ borderTop: '1px solid #2a2f3d', paddingTop: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                {QUICK_EARNINGS.filter(n => !earnings.find(c => c.name === n)).map(n => (
                  <button key={n} onClick={() => addComponent('earning', n)} style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 12, color: '#34d399', fontSize: 11, padding: '3px 10px', cursor: 'pointer' }}>+ {n}</button>
                ))}
              </div>
              {earnings.length === 0 && (
              ...
              {earnings.map((comp, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6 }}>
                    ...
                    <input type="number" min="0" style={{ ...inp, flex: 1, textAlign: 'right' }} placeholder={comp.calc_type === 'percent_of_basic' ? '%' : 'NPR'} value={comp.value} onChange={e => updateComponent(globalIdx, 'value', e.target.value)} />
                    {comp.calc_type === 'percent_of_basic' && basic > 0 && (
                      <span style={{ fontSize: 11, color: '#6b7280', whiteSpace: 'nowrap', minWidth: 60, textAlign: 'right' }}>= {computed.toLocaleString()}</span>
                    )}
            ...
            <div style={{ borderTop: '1px solid #2a2f3d', paddingTop: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                {QUICK_DEDUCTIONS.filter(n => !deductions.find(c => c.name === n)).map(n => (
                  <button key={n} onClick={() => addComponent('deduction', n)} style={{ background: 'rgba(248,113,113,0.06)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 12, color: '#f87171', fontSize: 11, padding: '3px 10px', cursor: 'pointer' }}>+ {n}</button>
                ))}
              </div>
              {basic > 0 && (
              ...
              <div style={{ borderTop: '1px solid #2a2f3d', paddingTop: 14 }}>
                ...
                <div style={{ background: '#0f1117', borderRadius: 8, border: '1px solid #2a2f3d', overflow: 'hidden' }}>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: r.big ? '12px 16px' : '7px 16px', borderTop: r.separator ? '1px solid #2a2f3d' : 'none', background: r.big ? 'rgba(201,168,76,0.05)' : 'transparent' }}>
