...
const HR_FEATURES = [
  ...
  {
    icon: '⚙', name: 'Pay Setup',
    guide: 'Configure and review salary, allowances, deductions, SSF, and bank details for every employee — all in one place. Click any employee row to open the Pay Setup form with two tabs: Salary and Bank/SSF. The Salary tab has three dedicated fields — Basic Salary, Dearness Allowance (महँगी भत्ता), and Other Allowances — plus deductions. The live Monthly Summary on the right shows the full breakdown: Basic → Dearness → Other Allowances → Gross → SSF deduction → Net (Cash in Hand) → Cost to Company (CTC). The compliance panel checks all three Nepal minimums simultaneously. The summary table shows Basic / Allowances / Gross / Deductions / Net / Employer SSF with totals and an Excel export (dearness and other allowances are separate columns). Daily and hourly staff show an estimated monthly cost (~rate × 26 days or × 8h × 26).',
    tips: ['Enter Basic Salary, Dearness Allowance, and Other Allowances as separate fields — dearness is stored and tracked independently per Nepal Labour Act', 'Nepal minimum wage (FY 2082/83): Basic ≥ NPR 12,170 · Dearness ≥ NPR 7,380 · Gross ≥ NPR 19,550/month — the compliance panel shows all three checks at once', 'SSF is computed on basic salary only (capped at NPR 100,000) — dearness and other allowances are excluded from the SSF base', 'Nepal Labour Act requires basic salary to be at least 60% of gross — an amber warning appears if it is lower', 'Cost to Company (CTC) = Gross + Employer SSF (20%) — the blue CTC row shows the employer\'s true monthly outlay', 'Net (Cash in Hand) = Gross − SSF − other deductions; TDS is not pre-computed here — it is applied during the Payroll run', 'Employer SSF (20%) is paid by the company and is not deducted from the employee\'s net salary', 'Bank details added here flow into payroll — employees without a bank account show an amber warning']
  },
  {
