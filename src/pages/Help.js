...
'Record overtime in ONE place — either the attendance sheet\'s OT column or the Overtime module. Both are paid, so the same hours entered in both pay twice; the payroll register flags affected employees with an ⚠ OT ×2? badge', 'TDS (income tax) is computed automatically from the fiscal-year tax slabs using year-to-date projection — finalize earlier months first so each month builds on the last; you can still override a value while the run is a draft', 'SSF contributors get the 1% first-slab social security tax waived, so most staff under roughly NPR 83,000/month gross pay zero income tax', 'Use the Payslip button on any row to view and print an individual payslip']
  ...
  },
  {
    ...
    guide: 'Gives employees a PIN-based login (no email/password) on their own phone to view their own payslips, submit leave requests, and check their roster — without needing access to the main admin app. Enable it per employee from Employees → Enable Self-Service, setting an initial PIN. Share the single login link via Employees → Copy Self-Service Link — the same link works for every self-service employee at your company; each picks their name and enters their PIN. An employee can only ever see their own data, never a coworker\'s — this is enforced at the database level, not just hidden in the UI.',
    tips: ['One shared link per company — copy it from Employees and send it to your team (e.g. via WhatsApp)', 'Leave requests submitted through self-service go through the exact same approval flow as leave submitted by an admin', 'Payslips only appear here once a payroll run is Finalized', 'A PIN gets locked for 15 minutes after 5 incorrect attempts, same protection as POS staff PINs', 'Re-run Enable Self-Service on an employee to reset their PIN']
  },
]
